...
def _build_input_with_defaults(args: argparse.Namespace) -> str:
    """Build input JSON, merging contract defaults with --input overrides."""
    root = get_project_root()
    contract_store = load_contract_store(root)
    pw = contract_store.get_pathway(args.pathway)
    ...
    if args.input:
        try:
            overrides = json.loads(args.input)
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    input_json = _build_input_with_defaults(args)

    if getattr(args, "force_mutate", False):
        _inject_broken_genes(orch)

    try:
        outputs = orch.run_pathway(args.pathway, input_json)
    except Exception as e:
        print(f"error: {e}", file=sys.stderr)
        sys.exit(1)
    finally:
...
def main() -> None:
    ...
    run_parser.add_argument("--input", default=None, help="input JSON string (uses contract defaults if omitted)")
