...
from sg.parser.types import (
    ...
    FieldDef, TypeDef, VerifyStep, FeedsDef, ConnectsDef,
...
class Parser:
    ...
    def parse_gene(self) -> GeneContract:
        ...
        while not self.at_end() and self.peek().type != TokenType.DEDENT:
            ...
            elif section == "connects":
                self.expect(TokenType.COLON)
                self._consume_newline()
                connects = self._parse_connects_block()
            else:
                self._skip_to_newline()
    ...
    def _parse_connects_block(self) -> list[ConnectsDef]:
        """Parse connects block: param_name  interface_type  "description" lines.

        interface_type is a free-form protocol/interface name chosen by the
        domain expert: https, sqlite, postgresql, kafka, s3, ftp, csv, etc.
        """
        items: list[ConnectsDef] = []
        if self.peek().type == TokenType.INDENT:
            self.advance()
    ...
    def _parse_field_defs(self) -> list[FieldDef]:
        ...
        Optional: type? (optional), type = default (before or after description)
        """
        ...
        if self.peek().type == TokenType.INDENT:
            ...
            while not self.at_end() and self.peek().type != TokenType.DEDENT:
                ...
                # default= can also appear after the description
                if default is None and self.peek().type == TokenType.IDENTIFIER and self.peek().value == "default":
                    self.advance()
                    if self.peek().type == TokenType.EQUALS:
                        self.advance()
                        default = self._read_value()
